...
const createKingdom = async (kingdomId: string, factionId: string, name: string, latitude: number, longitude: number) => {
  ...
  switch (factionId) {
    case 'black': {
      units = ['skeleton'];
      spells = ['animate-skeleton',
      'terror'];
      break;
    }
    case 'green': {
      units = ['goblin'];
      spells = ['summon-goblin', 'cure'];
      break;
    }
    case 'red': {
      units = ['orc'];
      spells = ['call-orc', 'fireball'];
      break;
    }
    case 'blue': {
      units = ['mage'];
      spells = ['summon-mage', 'ice-shock'];
      break;
    }
    case 'white': {
      units = ['pegasus'];
      spells = ['call-pegasus', 'healing-hand'];
      break;
    }
  }
  // troops
  ...
  spells.forEach(async (spell: any) => {
    const charm = (await angularFirestore.doc(`spells/${spell}`).get()).data();
    batch.create(angularFirestore.collection(`kingdoms/${kingdomId}/charms`).doc(), { id: charm?.id, spell: charm, turns: 0, completed: false })
  });
  // artifacts
...
const addCharm = async (kingdomId: string, spell: any, turns: number, batch: FirebaseFirestore.WriteBatch) => {
  ...
  if (kingdomCharm.size > 0) {
    ...
    batch.create(angularFirestore.collection(`kingdoms/${kingdomId}/charms`).doc(), { id: spell.id, spell: spell, turns: 0, assignment: 0, completed: false });
  }
}
...
const dischargeContract = async (kingdomId: string, contractId: string) => {
  ...
  if (kingdomContract) {
    await angularFirestore.doc(`kingdoms/${kingdomId}/contracts/${contractId}`).delete();
...
const taxGold = async (kingdomId: string, turns: number) => {
  ...
  if (turns <= kingdomTurn.quantity) {
    ...
    const kingdomFarm = (await angularFirestore.collection(`kingdoms/${kingdomId}/buildings`).where('id', '==', 'farm').limit(1).get()).docs[0].data();
    const gold = Math.floor((kingdomVillage.quantity + kingdomFarm.quantity) / 2 * kingdomVillage.structure.goldProduction * turns);
    const batch = angularFirestore.batch();
    await addSupply(kingdomId, 'turn', -turns, batch, kingdomTurn.resource.ratio);
    await addSupply(kingdomId, 'gold', gold, batch);
    await batch.commit();
...
const conjureCharm = async (kingdomId: string, charmId: string, targetId: string) => {
  let result = {};
  let charm = (await angularFirestore.doc(`kingdoms/${kingdomId}/charms/${charmId}`).get()).data();
  if (charm) {
    let kingdomTurn = (await angularFirestore.collection(`kingdoms/${kingdomId}/supplies`).where('id', '==', 'turn').limit(1).get()).docs[0].data();
    let kingdomMana = (await angularFirestore.collection(`kingdoms/${kingdomId}/supplies`).where('id', '==', 'mana').limit(1).get()).docs[0].data();
...
const activateArtifact = async (kingdomId: string, artifactId: string, targetId: string) => {
  let result = {};
  let artifact = (await angularFirestore.doc(`kingdoms/${kingdomId}/artifacts/${artifactId}`).get()).data();
  if (artifact && !artifact.item.battle) {
    ...
    if (artifact.quantity > 0 && artifact.item.turns <= max) {
      ...
      switch (artifact.item.subtype) {
        case 'summon':
          let unit = artifact.item.units[random(0, artifact.item.units.length - 1)];
          let size = random(Math.min(...unit.amount), Math.max(...unit.amount));
          await addTroop(targetId, unit, size, batch);
          result = { unit: `unit.${unit.id}.name`, size: size };
          break;
        case 'resource':
          let resource = artifact.item.resources[0];
          let amount = random(Math.min(...artifact.item.amount), Math.max(...artifact.item.amount));
...
const addBuilding = async (kingdomId: string, structureId: string, quantity: number, batch: FirebaseFirestore.WriteBatch) => {
  let kingdomBuilding = (await angularFirestore.collection(`kingdoms/${kingdomId}/buildings`).where('id', '==', structureId).limit(1).get()).docs[0];
  const building = kingdomBuilding.data();
  const q = building.quantity + quantity <= 0 ? 0 : admin.firestore.FieldValue.increment(quantity);
  batch.update(angularFirestore.doc(`kingdoms/${kingdomId}/buildings/${kingdomBuilding.id}`), { quantity: q });
}
...
const buyEmporium = async (kingdomId: string, itemId: string) => {
  let item = (await angularFirestore.doc(`items/${itemId}`).get()).data();
  if (item) {
    ...
    for (const kingdomAuction of kingdomAuctions.docs) { // cannot use forEach due to async/await of batch.commit
      ...
      if (moment().isAfter(moment(auction.auctioned.toMillis()))) {
        if (auction.kingdom) {
          ...
          if (auction.type === AuctionType.CHARM) await addCharm(auction.kingdom, auction.spell, auction.level, batch);
          ...
          from = {
            ...
            faction: {
              id: from?.faction.id,
              name: from?.faction.name,
              image: from?.faction.image,
            },
            power: from?.power,
          };
          */
...
const startAuction = async (type: AuctionType, batch: FirebaseFirestore.WriteBatch) => {
  ...
  switch (type) {
    case AuctionType.ARTIFACT:
      const item = (await angularFirestore.collection('items').where('random', '==', random(0, 49)).limit(1).get()).docs[0].data();
      batch.create(angularFirestore.collection('auctions').doc(), { type: AuctionType.ARTIFACT, item: item, quantity: random(1, 3), gold: 1, auctioned: auctioned, kingdom: null /*'wS6oK6Epj3XvavWFtngLZkgFx263'*/ });
      break;
    case AuctionType.CHARM:
      const spell = (await angularFirestore.collection('spells').where('random', '==', random(0, 100)).limit(1).get()).docs[0].data();
      batch.create(angularFirestore.collection('auctions').doc(), { type: AuctionType.CHARM, spell: spell, level: 0, gold: 1, auctioned: auctioned, kingdom: 'wS6oK6Epj3XvavWFtngLZkgFx263' });
      break;
    case AuctionType.CONTRACT:
      const hero = (await angularFirestore.collection('heroes').where('random', '==', random(0, 19)).limit(1).get()).docs[0].data();
      batch.create(angularFirestore.collection('auctions').doc(), { type: AuctionType.CONTRACT, hero: hero, level: random(1, 10), gold: 1, auctioned: auctioned, kingdom: 'wS6oK6Epj3XvavWFtngLZkgFx263' });
      break;
    case AuctionType.TROOP:
      const unit = (await angularFirestore.collection('units').where('random', '==', random(0, 64)).limit(1).get()).docs[0].data();
      batch.create(angularFirestore.collection('auctions').doc(), { type: AuctionType.TROOP, unit: unit, quantity: random(1, 1000), gold: 1, auctioned: auctioned, kingdom: 'wS6oK6Epj3XvavWFtngLZkgFx263' });
      break;
  }
}
...
const offerGod = async (kingdomId: string, godId: string, sacrifice: number) => {
  ...
  if (kingdomGod) {
      ...
      : kingdomGod.mana
        ...
        : kingdomGod.population
          ...
          : kingdomGod.land
            ? 'land'
            : 'turn';
    let kingdomSupply = (await angularFirestore.collection(`kingdoms/${kingdomId}/supplies`).where('id', '==', resource).limit(1).get()).docs[0].data();
    ...
    if (sacrifice >= kingdomGod.increment && sacrifice <= kingdomSupply.quantity) {
      ...
      let rewards = ['supply', 'artifact', 'contract', 'enchantment', 'troop', 'building', 'charm'];
